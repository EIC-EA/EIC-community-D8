...
class RequestDeleteRoutes {
  ...
  public function routes() {
    ...
    $delete_request_handler = $collector->getHandlerByType(
      RequestTypes::DELETE
    );
    ...
    foreach (
      array_keys(
        $delete_request_handler->getSupportedEntityTypes()
      ) as $entity_type
    ) {
      $route = (new Route(
        '/' . $entity_type . '/{' . $entity_type . '}/request-delete'
      ))
        ->addDefaults(
          [
        ...
        ->setRequirement(
          '_role',
          'trusted_user+administrator+content_administrator'
        );

      $route_collection->add(
        'entity.' . $entity_type . '.request_delete_form',
        $route
      );

      // Define the route which apply the given operation and close the request
      $route = (new Route(
        '/admin/community/' . $entity_type . '/{' . $entity_type . '}/request/{request_type}/close'
      ))
        ->addDefaults(
          [
      ...
      $route_collection->add(
        'entity.' . $entity_type . '.close_request',
        $route
      );
    }
