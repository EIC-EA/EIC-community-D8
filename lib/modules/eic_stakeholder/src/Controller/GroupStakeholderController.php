...
namespace Drupal\eic_stakeholder\Controller;
...
/**
 * Returns responses for 'group_stakeholder' GroupContent routes.
 */
class GroupStakeholderController extends GroupContentController {
  ...
  public function addPage(GroupInterface $group, $create_mode = FALSE) {
    ...
    $storage_handler = $this->entityTypeManager->getStorage('stakeholder_type');
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (!empty($build['#bundles'][$bundle_name])) {
        ...
        $t_args = ['%stakeholder_type' => $bundle_label];
        $description = $create_mode
          ? $this->t('Add a new stakeholder of type %stakeholder_type to the group.', $t_args)
          : $this->t('Add an existing stakeholder of type %stakeholder_type to the group.', $t_args);
  ...
  protected function addPageBundles(GroupInterface $group, $create_mode) {
    ...
    foreach ($plugin_ids as $key => $plugin_id) {
      if (strpos($plugin_id, 'group_stakeholder:') !== 0) {
