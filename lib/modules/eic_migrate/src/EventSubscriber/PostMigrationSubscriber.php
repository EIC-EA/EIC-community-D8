...
class PostMigrationSubscriber implements EventSubscriberInterface {
  ...
  /**
   * Updates a node's entity references field with passed values.
   *
   ...
   */
  protected function updateEntityReferenceValue(NodeInterface $node, string $field_name, array $values, bool $preserve_existing_values = TRUE) {
    if (!$preserve_existing_values) {
      $node->set($field_name, []);
    }

    $changed = $node->getChangedTime();
    $node->set($field_name, array_unique(array_merge($node->get($field_name)->getValue(), $values)));
