...
class GroupProjectsDashboardController extends ControllerBase {
  ...
  public function page(): array {
    ...
    if ($cache = $this->cache()->get('dashboard:projects')) {
      $content = $cache->data;
    }
    else {
      // Number of groups.
      ...
      // Groups evolution.
      $groupsCumulativeStats = $this->dashboardCumulativeService->getCumulativeStatsPerDashboardType(DashboardsDatabase::PROJECTS_DASHBOARD_TYPE);
      ...
      $groupsEvolution = $this->dashboardBuilder->chartLine($this->t('Projects - evolution over time'), $groupsData);
      ...
      $this->cache()->set('dashboard:projects', $content);
    }
