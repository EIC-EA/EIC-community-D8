...
class MembersDashboardController extends ControllerBase
{
  ...
  public function membersOverview(): array {
    // ===== Section 1.
    // Platform members.
    if ($section1Cache = $this->cache()->get('dashboard:members:section:1')) {
      ...
      $this->cache()->set('dashboard:members:section:1', $section1Build);
    }

    // ===== Section 2.
    // Members by country.
    if ($section2Cache = $this->cache()->get('dashboard:members:section:2')) {
      ...
      $this->cache()->set('dashboard:members:section:2', $section2Build);
    }

    // ===== Section 3.
    if ($section3Cache = $this->cache()->get('dashboard:members:section:3')) {
      ...
      $totalCompletedMembersProfilesData = json_encode([
        ...
        [
          ...
          'y' => ($totalMembers - $totalCompletedMembersProfiles)
        ],
      ], JSON_NUMERIC_CHECK);
      ...
      $membersLinkedToOrganisationsData = json_encode([
        [
          ...
          'y' => $membersLinkedToOrganisations
        ],
        [
          'name' => 'Not linked to any Organisation',
          'y' => ($totalMembers - $membersLinkedToOrganisations)
        ],
      ], JSON_NUMERIC_CHECK);
      ...
      $membersLinkedToProjectsData = json_encode([
        [
          'name' => 'Linked to at least one EIC Community organisation',
          'y' => $membersLinkedToProjects
        ],
        [
          'name' => 'Not linked to any Organisation',
          'y' => ($totalMembers - $membersLinkedToProjects)
        ],
      ], JSON_NUMERIC_CHECK);
      ...
      $this->cache()->set('dashboard:members:section:3', $section3Build);
    }

    // ===== Section 4.
    if ($section4Cache = $this->cache()->get('dashboard:members:section:4')) {
      ...
      $this->cache()->set('dashboard:members:section:4', $section4Build);
    }
