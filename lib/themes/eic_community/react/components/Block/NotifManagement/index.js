...
class Overview extends React.Component {
  ...
  showUnsubscribeModal(id, name, parentIndex, rowIndex, url) {
    ...
    search() {
        if (this.state.searchText.length > 0) {
            return this.state?.results?.items?.filter((item) => {
                return (!item.hasOwnProperty('hidden') || item.hidden === false) && item.name.label.toString()
                    .toLowerCase()
    ...
    axios.post(this.state.currentItemToUnsubscribeUrl, {withCredentials: true})
                    ...
                    <div className="ecl-subnavigation__searchform-wrapper">
                        <form className="ecl-search-form ecl-subnavigation__searchform" role="search">
                            <div className="ecl-form-group">
                                <input onChange={this.updateSearchText} id="input-search"
                            ...
                            <button className="ecl-button ecl-button--search ecl-search-form__button" type="submit"
                                    aria-label="Search">
          <span className="ecl-button__container"
                dangerouslySetInnerHTML={{__html: `<span class="ecl-button__label" data-ecl-label="true">${this.state.translations.search_placeholder}</span> ${svg('search', ' ecl-icon--xs ecl-button__icon ecl-button__icon--after')}`}}>
          </span>
                            </button>
                        </form>
                    </div>
                ...
                {this.state.loading &&
                    <CircularProgress style={{top: '50%', left: '50%', position: 'fixed', zIndex: '51'}}
                                      className="loader" size={50}/>}
                <table className="ecl-my-notifications-management__table" cellPadding="0" cellSpacing="0">
                    ...
                    {(this.state.loading === false && this.state.searchText !== '' && visibleResults.length === 0) &&
                        <tr>
                            <td colspan={this.props.unsubscribe ? 3 : 2}
          ...
          >
            <span className="ecl-link__label">{this.state.translations.unsubscribe_modal_close}</span>&nbsp;
              <svg className="ecl-icon ecl-icon--2xs ecl-link__icon" focusable="false" aria-hidden="true">
                        ...
                        className="ecl-activity-stream__modal__title">{this.state.translations.unsubscribe_modal_title}</h3>
