...
class Overview extends React.Component {
  ...
  render() {
    if (this.state.loading)
      return <></>
    let visibleResults = this.state?.results?.items?.filter((item) => !item.hasOwnProperty('hidden') || item.hidden === false);
      ...
      <>
        ...
        <table className="ecl-my-notifications-management__table" cellPadding="0" cellSpacing="0">
          ...
          {visibleResults.length > 0 &&
            this.state.results.items.map((row, index) => {
              return (
                <NotifRow hidden={row.hidden} rowIndex={index} row={row} key={index} unsubscribe={this.props.unsubscribe} showUnsubscribeModal={this.showUnsubscribeModal} translations={this.state.translations} />
              )
            })
          }
          ...
          {(this.state.loading === false && this.state.searchText !== '' && visibleResults.length === 0) &&
            <tr>
              <td colspan={this.props.unsubscribe ? 3 : 2} className="ecl-my-notifications-management__table__no-results">
