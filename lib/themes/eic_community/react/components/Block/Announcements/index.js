    ...
    <div className={'ecl-organisation__announcements__item'}>
      ...
      <div>{item.items.slice(0, load + 2).map((annoucement, index) => <AnnoucementsItem item={annoucement} key={index} />)}</div>
      {
        ...
        <button
          ...
          dangerouslySetInnerHTML={{__html: 'Show 2 more ' + svg('arrow', 'ecl-icon--xs')}}
        >
          {/*<span className="ecl-button__label" data-ecl-label="true">Show 2 more </span>*/}
          {/*<span dangerouslySetInnerHTML={{__html: svg('arrow', 'ecl-icon--xs')}} />*/}
        </button>
      }
    </div>
  )
