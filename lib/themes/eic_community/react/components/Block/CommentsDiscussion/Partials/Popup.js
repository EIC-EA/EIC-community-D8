...
class PopupForm extends React.Component {
  ...
  submit(e, closeFunction) {
    closeFunction();
    this.props.updateComment(e, this.props.comment.comment_id, this.state.text)
  }

  render() {
    return <Popup
      trigger={<span className="ecl-link ecl-link--default ecl-link--button ecl-link--button-ghost ecl-collapsible-options__button ecl-comment__edit"
                  data-comment-id="1">Edit comment</span>}
      modal
      nested
    >
      {close => (
                    ...
                    <form onSubmit={(e) => this.submit(e, close)}>
                      <div className="ecl-form-group">
                        <label className="ecl-comment-form__textarea-label ecl-form-label"
                               htmlFor="ecl-comment-form-reply">Edit</label>
