...
import {hasPermission, hasPermissionApi} from "../Services/Permissions";
...
class Answer extends React.Component {
  ...
  render() {
    ...
    if (parseInt(child.is_soft_delete) !== 0) {
      const date = new Date(Date.parse(child.deletion_date));
          ...
          <div className="ecl-comment__content">
            ...
            <div className="ecl-comment__main">
              <div className="ecl-editable-wrapper">
                <b>{`${child.text} ${date.getDate()} ${getMonthByIndex(date.getMonth())} ${date.getFullYear()}`}</b>
              </div>
