...
import { hasPermission, hasPermissionApi, isCommentHidden } from '../Services/Permissions';
import TimeStatus from './TimeStatus';
import Author from './components/Author';
import TaggedUsers from './TaggedUsers';
import TopContrib from './components/TopContrib';
  ...
  render() {
    ...
    return (
      <div className="ecl-comment-thread">
        <div
          className="ecl-comment ecl-comment--is-reply ecl-comment-thread__comment"
          id={child.comment_id}
        >
          <Author url={child.user_url} avatar={child.user_image} fullname={child.user_fullname} />
          <div className="ecl-comment__content">
            {parseInt(child.is_soft_delete) === 0 && (
              <header className="ecl-comment__header">
                <div className="ecl-comment__header-main">
                  <div className="ecl-comment__author-info">
                    <a
                      href={child.user_url}
                      className="ecl-link ecl-link--standalone ecl-comment__author-name"
                    >
                      {child.user_fullname}
                    </a>{' '}
                    {child.user_top_contributor && <TopContrib toolTip={false} label={child.user_top_contributor.label} />}
                    <span className="ecl-comment__origin ecl-u-ml-2xs">
                      {getTranslation('in_reply_to')}&nbsp;
                    </span>
                    <a
                      href={`${this.props.parent.user_url}`}
                      className="ecl-link ecl-link--standalone ecl-comment__author-name"
                    >
                      {this.props.parent.user_fullname}
                    </a>{' '}
                    {this.props.parent.user_top_contributor && <TopContrib toolTip={false} label={this.props.parent.user_top_contributor.label} />}
                  </div>
            ...
            <div className="ecl-comment__main">
              <div className="ecl-editable-wrapper" dangerouslySetInnerHTML={{__html: isCommentHidden(child) ? `<b>${child.text}</b>` : child.text}}/>
                        ...
                        <div className="ecl-comment__action">
                          <CommentAction>
                            <div>
                              ...
                              >
                                <span className="ecl-link__label">{`${getTranslation(
                                  'reply_to'
                                )}`}</span>
                              </span>
                            </div>
            ...
            {!isCommentHidden(child) && this.props.activeAnswerForm === child.comment_id && (
              <CommentForm
