...
class CommentsDiscussion extends React.Component {
  ...
  componentDidMount() {
    this.fetchComments();
    if (document.getElementById('ecl-comment-overview__wrapper') && this.props.highlightedCommentId != 0) {
      document.getElementById('ecl-comment-overview__wrapper').scrollIntoView()
    }
  }
  ...
  addComment(e, parentId = null, taggedUsers = []) {
    ...
    axios.post(getPrefixDiscussionEndpoint() + '/' + this.props.discussionId + '/comment', {
      ...
      .then(function (response) {
        ...
        self.addNotification(getTranslation('comment_added'))
        self.fetchComments();
      }.bind(this))
  ...
  deleteComment(e, commentId, text, parentId = null) {
    ...
    axios.delete(getPrefixDiscussionEndpoint() + '/' + window.drupalSettings.overview.group_id + '/' + this.props.discussionId + '/delete/' + commentId,
      { withCredentials: true })
      .then(function (response) {
        self.addNotification(getTranslation('comment_deleted'))
        this.setState({
          childToReload: parentId,
        });
        self.fetchComments();
  ...
  flagComment(commentId, type, flagId, text = null) {
    ...
    axios.post(getPrefixDiscussionEndpoint() + '/' + this.props.discussionId + '/' + flagId + '/' + commentId + '/' + type,
      ...
      .then(function (response) {
        ...
        self.addNotification(translation)
        self.fetchComments();
      }.bind(this))
      .catch(function (error) {
        self.addNotification('An error occured', 'danger')
      ...
      .then(response => {
        if (response.data && response.data.result) {
          ...
          if (request_type === 'archive') {
            translation = getTranslation('comment_request_archived')
          }
          if (request_type === 'delete') {
            translation = getTranslation('comment_request_deleted')
          }
          if (translation.length <= 0) {
            translation = getTranslation('action_request_default')
          }
  ...
  updateComment(e, commentId, text, parentId = null, taggedUsers=[]) {
    e.preventDefault();
    const self = this;
    this.setState({
      loading: true
    ...
    axios.patch(getPrefixDiscussionEndpoint() + '/' + this.props.discussionId + '/edit/' + commentId,
      { 'text': text, taggedUsers: taggedUsers, withCredentials: true })
      .then(function (response) {
        self.addNotification(getTranslation('comment_edited'))
        this.setState({
          childToReload: parentId,
        });
        self.fetchComments();
  ...
  render() {
    return <div className="ecl-comment-overview__wrapper">
      ...
      <div ref={this.wrapperRef} id='ecl-comment-overview__wrapper' className="ecl-comment-overview__items">
