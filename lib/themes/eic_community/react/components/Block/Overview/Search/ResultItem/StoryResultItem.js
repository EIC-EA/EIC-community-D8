...
export default function StoryResultItem(props) {
  ...
  // Public/Private label on News overview page: Uncomment to enable along with corresponding code on /modules/eic_search/src/Search/DocumentProcessor/ProcessorNewsStory.php
  ...
  const storyTypeId = props.result.its_content_field_vocab_story_type;
  const allowedStoryTypeIds = (window.drupalSettings && Array.isArray(window.drupalSettings.storyTypes))
    ? window.drupalSettings.storyTypes
    : []; // Type "Call" (Open call / Partner Call)
  const showStoryTypeTag = allowedStoryTypeIds.includes(storyTypeId);
  const contentTypeLabel = showStoryTypeTag ? 'call' : 'news';
        ...
        <div className="ecl-teaser__meta-header">
          <div className="ecl-teaser__meta-column">
            <div className="ecl-content-type-indicator ecl-teaser__type">
              <div dangerouslySetInnerHTML={{__html: svg(contentTypeLabel, 'ecl-icon ecl-icon--m ecl-content-type-indicator__icon')}}/>
              <span className="ecl-content-type-indicator__label">{contentTypeLabel}</span>
            </div>
            ...
            <span className="ecl-tag ecl-tag--display">
              {props.result.ss_content_field_story_type_string}
            </span>
          </div>
          ...
          <div className="ecl-author ecl-author--is-default  ecl-teaser__author">
            ...
            <div className="ecl-author__aside">
              <UserImage
                ...
                src={!props.isAnonymous && props.result.ss_content_author_formatted_image ? url(props.result.ss_content_author_formatted_image) : null}
