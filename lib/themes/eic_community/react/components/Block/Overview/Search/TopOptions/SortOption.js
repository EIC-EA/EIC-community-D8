...
class SortOption extends React.Component {
  constructor(props) {
    super(props);
    const defaultSortOption = window.drupalSettings.overview.default_sorting_option;
    this.state = {
      value: defaultSortOption.length > 0 ? defaultSortOption[0] + '__' + defaultSortOption[1] : 'any'
    };
          ...
          >
            ...
            {Object.entries(this.props.sortSettings).map((value) => {
              ...
              return <React.Fragment key={value[1]} >
                ...
                {typeof this.translations.sources[this.sourceBundle].sort[descKey].length !== "undefined" && <option value={descKey}>{`${this.translations.sources[this.sourceBundle].sort[descKey]}`}</option>}
              </React.Fragment>
            })}
          </select>
