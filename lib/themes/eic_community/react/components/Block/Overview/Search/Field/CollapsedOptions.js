...
const CollapsedOptions = ({values, facetsValue, bundle, updateFacet, translations}) => {
  ...
  // @TODO Add children checkboxes.
  for (i = 0; i <= facets.length; i++) {
    let currentFacet = facets[i]
    if (!currentFacet) {
      continue
    }
    let splitFacets = Object.values(currentFacet)[0].split('___')

    if (splitFacets.length > 1) {
      let key = Object.keys(facets).find(key => facets[key][0] === splitFacets[1])
      if (facets.key === undefined) {
        continue
      }
      facets[key].children = facets[key].children ?? []
      facets[key].children.push([splitFacets[0], currentFacet[1]])
      facets.splice(i, 1)
    }
  }
