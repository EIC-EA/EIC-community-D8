...
const CollapsedOptions = ({values, facetsValue, bundle, updateFacet}) => {
  ...
  // TODO: change string by translation key
  const action = '<span className="ecl-button__label" data-ecl-label="true">'+ (isHide ? "Show more" : "Collapse") +'</span>' + svg('arrow-down', `ecl-icon ecl-icon--xs ${isHide && 'ecl-icon--rotate-180'} ecl-button__icon ecl-button__icon--after`)
