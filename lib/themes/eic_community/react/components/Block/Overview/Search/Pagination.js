  ...
  return (
    <ul className="ecl-pagination__list">
      {items.map(({ page, type, selected, ...item }, index) => {
        ...
        return <li {...item} className={`ecl-pagination__item ${type === 'previous' && 'ecl-pagination__item--previous'} ${type === 'next' && 'ecl-pagination__item--next'}  ${selected ? 'ecl-pagination__item--current' : ''}`} key={index}>{children}</li>;
      })}
    </ul>
  )
}
