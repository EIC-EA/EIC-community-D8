...
class UserGalleryResultItem extends React.Component {
  ...
  getLastActivity() {
    ...
    const date = new Date(this.props.result.ds_user_changed).getTime()
    const now = Date.now()
  ...
  render() {
    ...
    const roles = {
      owner: `${svg('Group owner')}<span class="ecl-content-type-indicator__label">&nbsp;${    this.props.translations.groupOwner || 'Group owner'}</span>`,
      admin: `${svg('Group administrator')}<span class="ecl-content-type-indicator__label">&nbsp;${    this.props.translations.groupOwner || 'Group administrator' }</span>`,
