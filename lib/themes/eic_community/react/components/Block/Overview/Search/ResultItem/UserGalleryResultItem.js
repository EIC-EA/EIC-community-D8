...
import TopContrib from "../../../CommentsDiscussion/Partials/components/TopContrib";
...
class UserGalleryResultItem extends React.Component {
  ...
  getLastActivity() {
    ...
    let date = new Date(this.props.result.ds_user_access).getTime()
    if (0 === date) {
      date = new Date(this.props.result.ds_user_created).getTime()
    }
    const now = Date.now()
        ...
        <div className="ecl-teaser ecl-teaser--member ecl-teaser--as-card ecl-teaser--as-card-grey">
          <div className="ecl-teaser__main-wrapper">
            <div className="ecl-teaser__meta-header" aria-hidden="true">
              ...
              />
              {this.props.result.bs_field_top_contributor && (
                <div className="ecl-content-type-indicator ecl-teaser__type">
                  <TopContrib toolTip={false} label={'Top contributor'} />
                </div>
              )}
            </div>
            <a href={itemUrl}>
              {this.props.result.ss_user_formatted_image
                ?
                <figure className="ecl-teaser__image-wrapper">
                  <img className="ecl-teaser__image" src={url(this.props.result.ss_user_formatted_image)} alt=""/>
                </figure>
                :
                <figure
