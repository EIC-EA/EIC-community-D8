...
(function (ECL, Drupal) {
  ...
  const isVisible = (el) => {
    ...
    return (
      el.contains(document.elementFromPoint(left, top)) ||
      el.contains(document.elementFromPoint(right, top)) ||
      el.contains(document.elementFromPoint(right, bottom)) ||
      el.contains(document.elementFromPoint(left, bottom))
    );
  };
})(ECL, Drupal);
