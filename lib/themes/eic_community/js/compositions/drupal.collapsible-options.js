...
(function (Drupal) {
   ...
   * @param {Number} index Implements the logic from the given index within
   * NodeList.
   */
   ...
   * instance expands.
   */
  function toggle(collapsibleOptions, context) {
    if (collapsibleOptions.getAttribute('aria-expanded') == 'true') {
      collapse(collapsibleOptions);
    } else {
      expand(collapsibleOptions, context);
    }
  }
   ...
   */
  function expand(collapsibleOptions, context) {
