  ...
  const positionDropdown = (context, target, corner, helper) => {
    ...
    if (y > 0) {
      if (y > context.scrollHeight / 2 + context.offsetHeight / 2) {
        ...
        corner.style.marginTop = `${0 - context.offsetHeight / 2 - corner.scrollHeight / 2}px`;
      } else {
        console.log('aa');
        target.style.top = `${0 - y}px`;
        corner.style.top = `${corner.scrollHeight / 2}px`;
        corner.style.marginTop = `${y}px`;
      }
    }
